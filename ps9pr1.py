...
class Board:
    ...
    def add_checker(self, checker, col):
        ...
        for row in range(self.height):
            if self.slots[-1*row - 1][col] == ' ':
                self.slots[-1*row - 1][col] = checker
                break
        ...
        for row in range(self.height - 3):
            for col in range(self.width -3):
                if self.slots[row][col] == checker and \
                   ...
                   self.slots[row + 3][col + 1] == checker:
                       return True
        return False
    ...
    def is_win_for(self, checker):
        """ accepts a parameter checker that is either 'X' or 'O', and returns True if there 
            are four consecutive slots containing checker on the board. Otherwise, it should return False.
        """
        assert(checker == 'X' or checker == 'O')
